    ...
    <Content id="main-content" className="dashboard">
      ...
      {page === "routing" && <section aria-label="Routing health" className="dashboard-section dashboard-section--page"><div className="page-context"><Tag type="gray">{range} window</Tag></div>
        <div className="monitoring-grid">
          ...
          <div className="span-12"><Panel title="Session history" description="Each cell is a sampled state, not a continuous uptime guarantee. Gaps remain unfilled."><StateHistory data={current} metric="sessionHistory" loading={loading && !current} include={(labels) => protocols.some((protocol) => protocol.name === labels.protocol && protocol.location === labels.location)} /></Panel></div>
